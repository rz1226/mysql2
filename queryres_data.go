...
func (q *QueryRes) ToStruct(dstStructs interface{}) error {
	...
	if isMany {
		return queryRes2StructBatch(data, dstStructs)
	}
	return queryRes2Struct(data[0], dstStructs)
}

//  map数据映射到struct, 同名映射到struct的key不区分大小写，检查类型  。dstStruct是接收数据的struct的指针
// 可以加多个struct接收数据, 一般只支持 整数，浮点，字符串
//  第二个参数是struct 指针
func queryRes2Struct(sourceData map[string]interface{}, dstStruct interface{}) error {

	return _structFromQueryRes(sourceData, dstStruct)

}

// 第二个参数是&[]*SomeStruct
func queryRes2StructBatch(sourceDatas []map[string]interface{}, dstStructs interface{}) error {
	...
	for _, v := range sourceDatas {
		...
		err := _structFromQueryRes(v, eleData)
		if err != nil {
			return err
		}
...
func _structFromQueryRes(sourceData map[string]interface{}, dstStruct interface{}) (resErr error) {
